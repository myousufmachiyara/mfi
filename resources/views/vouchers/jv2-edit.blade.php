										...
										<table class="table table-bordered table-striped mb-0" id="myTable" >
											...
											<tbody id="JV2Table">
                                            	@foreach ($jv2_items as $jv_key => $jv2_items)
													<tr>
														<td>
															<select data-plugin-selectTwo class="form-control" autofocus name ="account_cod[]" onchange="addNewRow()" required>
...
<script>
	...
	function addNewRow(){
		...
		if(latestValue!=""){
			...
			cell1.innerHTML  = '<select "data-plugin-selectTwo" class="form-control" autofocus onclick="addNewRow()" name ="account_cod[]" required>'+
									'<option value="" disabled selected>Select Account</option>'+
									@foreach($acc as $key => $row)	
