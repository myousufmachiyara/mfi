...
class RptAccGrpBAController extends Controller
{
    ...
    private function bageneratePDF($balance_all, Request $request){
        ...
        foreach ($groupedData as $headCount => $heads) {
            // Initialize totals for each head group
            $totalDebitForHead = 0;
            $totalCreditForHead = 0;
            ...
            foreach ($heads as $subHeadCount => $subheads) {
                ...
                foreach ($subheads as $item) {
                    ...
                    // Update totals for the head group
                    $totalDebitForHead += $item['Debit'];
                    $totalCreditForHead += $item['Credit'];
            ...
            // Add total row for this head group
            $html .= '<tr style="background-color:#d9edf7; font-weight:bold;">
                        <td colspan="4" style="text-align:right;"><strong>Total For ' . $headCount . '</strong></td>
                        <td>' . number_format($totalDebitForHead, 0) . '</td>
                        <td>' . number_format($totalCreditForHead, 0) . '</td>
                    </tr>';
        }
