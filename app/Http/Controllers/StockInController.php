...
class StockInController extends Controller
{
    ...
    public function generatePDF($id)
    {
        ...
        $stock_in_items = stock_in_2::where('sales_inv_cod',$id)
                ->join('item_entry','stock_in_2.item_cod','=','item_entry.it_cod')
                ->select('stock_in_2.*','item_entry.item_name')
                ->get();
        ...
        $pdf->SetAuthor('MFI');
        $pdf->SetTitle('Stock In-'.$stock_in['prefix'].$stock_in['Sal_inv_no']);
        $pdf->SetSubject('Stock In-'.$stock_in['prefix'].$stock_in['Sal_inv_no']);
        $pdf->SetKeywords('Stock In, TCPDF, PDF');
        ...
        $heading='<h1 style="font-size:20px;text-align:center;font-style:italic;text-decoration:underline;color:#17365D">Stock In Doors</h1>';
        ...
        $html .= '<td  style="font-size:10px;font-family:poppins;width: 85%;">'.$stock_in['ac_name'].'</td>';
        $html .= '</tr>';
        $html .= '<tr>';
        $html .= '<td style="font-size:10px;font-weight:bold;font-family:poppins;width: 15%;color:#17365D">Remarks</td>';
        $html .= '<td  style="font-size:10px;font-family:poppins;width: 85%;">'.$stock_in['Sales_remarks'].'</td>';
        $html .= '</tr>';
        $html .= '</table>';
        ...
        foreach ($stock_in_items as $items) {
