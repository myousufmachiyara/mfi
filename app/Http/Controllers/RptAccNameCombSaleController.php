...
class RptAccNameCombSaleController extends Controller
{
    ...
    {
        $pur_by_account = both_sale_rpt_by_account::where('ac1', $request->acc_id)
            ->whereBetween('date', [$request->fromDate, $request->toDate])
        ...
        $pdf->SetAuthor('MFI');
        $pdf->SetTitle('All Sale Report Of Account '.$request->acc_id);
        $pdf->SetSubject('All Sale Report Of Account '.$request->acc_id);
        $pdf->SetKeywords('All Sale Report Of Account, TCPDF, PDF');
        ...
        $heading = '<h1 style="font-size:20px;text-align:center;font-style:italic;text-decoration:underline;color:#17365D">All Sale Report Of Account</h1>';
